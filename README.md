...
**许可：[PolyForm Noncommercial 1.0.0](LICENSE)。** 非商业及条款明确允许的用途可用，一般商业用途需另行授权，包含商业接单。本项目属于源码可用项目；[先前 MIT 版本的权利仍有效](NOTICE.md)。
...
**演示视频待补充。** 首次发布仅包含 Skill、方法库、模板和脚本，没有上传效果视频，也没有用其他作品代替本项目的实际输出。
...
- 当前代码与方法文档采用 [PolyForm Noncommercial License 1.0.0](LICENSE)，保留官方原文，不添加商业接单例外。非商业及条款明确允许的用途可用，商业用途需另行授权；机构用途按许可证原文判断。
- 商业授权申请与历史 MIT 权利边界见 [NOTICE.md](NOTICE.md)。本次换证不能追溯限制此前已按 MIT 发布的核心 Skill 与脚本。
- 外部模型、品牌、字体、音乐和视频素材的权利不因本仓库公开而转移；展示案例应单独注明来源与授权。
